        ...
        {view==="add" ? (
          <div>
            ...
            <div style={{ marginBottom:12 }}>
              ...
              <textarea value={form.note} onChange={e=>sf("note",e.target.value)} placeholder="e.g. Swap March creatives for April — check with client first" style={{ ...iS, resize:"vertical", minHeight:70, lineHeight:1.5 }}/>
            </div>
      ...
      <div style={{ display:"flex", alignItems:"center", gap:10 }}>
        ...
        <div>
          ...
          <div style={{ color:"#92400e", fontSize:11, marginTop:1 }}>
            {due.slice(0,2).map(r=>{
              const rt = REMINDER_TYPES.find(t=>t.value===r.type)||REMINDER_TYPES[5];
              return <span key={r.id} style={{ marginRight:10 }}>{rt.label}{r.note ? ` — ${r.note.slice(0,40)}${r.note.length>40?"…":""}` : ""}</span>;
            })}
      ...
      <td colSpan={colSpan} style={{ padding:0, borderBottom:"1px solid #0d1525" }}>
        <div style={{ background:"#07101c", borderTop:"1px solid #1a2744", padding:"16px 16px 16px 52px" }}>
          ...
          <div style={{ display:"flex", alignItems:"center", gap:10 }}>
            ...
            {!dirty && (c.impressions||c.ctr||c.cpm||c.spend) && <span style={{ fontSize:11, color:"#00d48a", display:"flex", alignItems:"center", gap:4 }}><span>✓</span><span>Metrics saved</span></span>}
            {(local.impressions||local.ctr||local.cpm||local.spend) && (
              <button onClick={()=>{ setLocal({impressions:"",ctr:"",cpm:"",spend:""}); setDirty(true); }} style={{ background:"none", border:"none", color:"#3d5a72", fontSize:11, cursor:"pointer" }}>Clear all</button>
            )}
          </div>
        </div>
      </td>
...
function DateBar({ range, setRange }) {
  ...
  function handleDropdown(val) {
    ...
    else if (val!=="__none__") { setShowCustom(false); const p=presets[val]; setRange({preset:val,...p}); }
  }
  return (
    <div style={{ background:"#0c1625", border:"1px solid #1e293b", borderRadius:10, padding:"11px 16px", marginBottom:14, display:"flex", flexWrap:"wrap", alignItems:"center", gap:8 }}>
      ...
      <div style={{ display:"flex", gap:5 }}>
        {quickKeys.map(k => {
          ...
          return <button key={k} onClick={()=>{ setShowCustom(false); setRange({preset:k,...presets[k]}); }} style={{ background:on?"#002e24":"#0e1a2e", border:`1px solid ${on?"#00c896":"#162236"}`, borderRadius:6, padding:"4px 13px", color:on?"#00e5a0":"#4d6e8a", fontSize:12, fontWeight:on?700:500, cursor:"pointer" }}>{presets[k].label}</button>;
        })}
      </div>
      ...
      <div onClick={e=>e.stopPropagation()} style={{ background:"#0e1a2e", border:"1px solid #1e293b", borderRadius:12, padding:24, width:"min(500px,95vw)", maxHeight:"90vh", overflowY:"auto", boxShadow:"0 30px 80px rgba(0,0,0,.9)" }}>
        ...
        <div style={{ marginBottom:12 }}>
          ...
          <div style={{ display:'flex', gap:6 }}>
            ...
            {f.folderPath && f.folderPath.trim() && <button onClick={()=>navigator.clipboard.writeText(f.folderPath.trim())} title='Copy path' style={{ background:'#002e24', border:'1px solid #00c89640', borderRadius:6, padding:'7px 12px', color:'#00e5a0', fontSize:12, fontWeight:600, cursor:'pointer', whiteSpace:'nowrap' }}>Copy 📋</button>}
          </div>
          {f.folderPath && f.folderPath.trim() && (
            <div style={{ marginTop:5, fontSize:10, color:'#3d5a72', fontStyle:'italic' }}>💡 Click Copy, then paste into Windows Explorer address bar (Ctrl+L) to open</div>
          )}
        </div>
        <div style={{ marginBottom:16 }}>
...
export default function App() {
  ...
  const [reminders, setReminders] = useState(() => {
    try { const s = localStorage.getItem(REMINDERS_KEY); return s ? JSON.parse(s) : []; }
    catch { return []; }
  });
  ...
  return (
    <div style={{ minHeight:"100vh", background:"#070d16", fontFamily:"'Inter','Segoe UI',system-ui,sans-serif", color:"#d8eaf8", fontSize:15 }}>
        ...
        <div style={{ maxWidth:1600, margin:"0 auto", display:"flex", justifyContent:"space-between", alignItems:"center", flexWrap:"wrap", gap:10 }}>
          ...
          <div style={{ display:"flex", gap:7, flexWrap:"wrap" }}>
            <button onClick={()=>setShowReminderModal(true)} style={{ position:"relative", background:pendingReminders>0?"#130a00":"#0e1a2e", border:`1px solid ${pendingReminders>0?"#f59e0b60":"#1e293b"}`, borderRadius:7, padding:"6px 13px", color:pendingReminders>0?"#f59e0b":"#4d6e8a", fontWeight:600, fontSize:13, cursor:"pointer", display:"flex", alignItems:"center", gap:6 }}>
              🔔 Reminders
              {pendingReminders > 0 && (
                <span style={{ background:"#ef4444", color:"#fff", borderRadius:10, padding:"0px 5px", fontSize:10, fontWeight:700, minWidth:16, textAlign:"center" }}>{pendingReminders}</span>
              )}
            </button>
      ...
      <div style={{ maxWidth:1600, margin:"0 auto", padding:"18px 20px 40px" }}>
        ...
        <ReminderAlertBanner
          reminders={reminders}
          onOpen={()=>setShowReminderModal(true)}
          onDismissAll={()=>setReminders(prev=>prev.map(r=>r.date<=today?{...r,dismissed:true}:r))}
        />
            ...
            <table style={{ width:"100%", borderCollapse:"collapse", minWidth:920 }}>
              ...
              <tbody>
                {filtered.map((c,i)=>{
                  ...
                  return (
                    <>
                      <tr key={c.id} className="crow" draggable
                        onDragStart={()=>onDragStart(c.id)} onDragOver={e=>onDragOver(e,c.id)}
                        onDrop={e=>onDrop(e,c.id)} onDragEnd={onDragEnd}
                        ...
                        <td style={{ padding:"0 0 0 8px", borderBottom:"1px solid #060c18", textAlign:"center", verticalAlign:"middle", width:36 }}>
                          <button onClick={()=>toggleExpand(c.id)} className="xbtn" title={open?"Collapse":"Expand"} style={{ background:"none", border:"none", cursor:"pointer", padding:"5px 6px", color:hasData?"#00c896":"#1e3048", transform:open?"rotate(90deg)":"rotate(0deg)", fontSize:11, lineHeight:1, display:"block", margin:"0 auto" }}>▶</button>
                        </td>
                        ...
                        <TD>
                          <div style={{ display:"flex", alignItems:"center", gap:5 }}>
                            ...
                            {c.monthlyFlight && <button onClick={()=>updateCampaign({...c,monthlyFlight:false})} title="Monthly flights" style={{ background:"none", border:"none", padding:0, cursor:"pointer", color:"#00e5c0", fontSize:13, lineHeight:1, flexShrink:0 }}>★</button>}
                            {!c.monthlyFlight && <button onClick={()=>updateCampaign({...c,monthlyFlight:true})} title="Mark as monthly flights" style={{ background:"none", border:"none", padding:0, cursor:"pointer", color:"#1e3048", fontSize:13, lineHeight:1, flexShrink:0, opacity:0 }} className="star-toggle">★</button>}
                            {campReminders.length > 0 && (
                              <button onClick={()=>setShowReminderModal(true)} title={`${campReminders.length} pending reminder${campReminders.length>1?"s":""}`} style={{ background:"#f59e0b20", border:"1px solid #f59e0b60", borderRadius:10, padding:"1px 6px", fontSize:10, color:"#f59e0b", fontWeight:700, cursor:"pointer", flexShrink:0 }}>
                                🔔 {campReminders.length}
                              </button>
                            )}
                            {c.note2 && c.note2.trim() && (
                              <span title={c.note2.trim()} style={{ background:"#200808", border:"1px solid #ef444460", borderRadius:3, padding:"1px 5px", fontSize:9, color:"#ef4444", fontWeight:700, letterSpacing:"0.05em", whiteSpace:"nowrap", flexShrink:0, cursor:"default" }}>⚠ {c.note2.trim().length>18?c.note2.trim().slice(0,18)+"…":c.note2.trim()}</span>
                            )}
                          </div>
                          {c.note1 && c.note1.trim() && (
                            <div style={{ fontSize:11, color:"#00ffb3", marginTop:3, fontWeight:500, whiteSpace:"nowrap", overflow:"hidden", textOverflow:"ellipsis", maxWidth:220 }} title={c.note1}>{c.note1.trim()}</div>
                          )}
                          {!open && hasData && (
